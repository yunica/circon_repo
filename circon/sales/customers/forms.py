...
class ChangePasswordForm(forms.Form):
    current_password = forms.CharField(label='current password', min_length=5,
                                       widget=forms.PasswordInput(
                                           attrs={'class': 'form-control'})
                                       )
    password = forms.CharField(label='New password', min_length=5,
                               widget=forms.PasswordInput(
                                   attrs={'class': 'form-control'})
                               )
    password_two = forms.CharField(label='Repeat password', min_length=5,
                                   widget=forms.PasswordInput(
                                       attrs={'class': 'form-control'})
                                   )
